...
import net.majorkernelpanic.AngleHandler;
import net.majorkernelpanic.SerialConsoleActivity;
...
public class RequestHandler {
	...
	static private void exec(JSONObject object, StringBuilder response) throws JSONException, IllegalArgumentException, IllegalAccessException {
		...
		// moves
		else if (action.equals("moves")) {
			String dir=(object.getString("name"));
			if(dir.equals("up")){
				AngleHandler.upAngle();
				try {
					SpydroidActivity.sendData(AngleHandler.setAngle());
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
			else if(dir.equals("down")){
				AngleHandler.downAngle();
				try {
					SpydroidActivity.sendData(AngleHandler.setAngle());
				} catch (IOException e) {
					e.printStackTrace();
