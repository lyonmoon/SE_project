...
public class SpydroidActivity extends FragmentActivity {
	...
	private final String TAG2 = SerialConsoleActivity.class.getSimpleName();
	...
	private final SerialInputOutputManager.Listener mListener =
			new SerialInputOutputManager.Listener() {
				...
				public void onNewData(final byte[] data) {
					SpydroidActivity.this.runOnUiThread(new Runnable() {
						@Override
						public void run() {
							SpydroidActivity.this.updateReceivedData(data);
                            /*try {
                                sendData();
                            } catch (IOException e) {
                                e.printStackTrace();
	...
	private void startIoManager() {
		if (sPort != null) {
			...
			byte[] tempangle="000000".getBytes();
			try {
				sPort.write(tempangle,250);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
	...
	//when get angle value from other device, Must be given this method

	public static void sendData(String string) throws IOException {
		byte[] angle="999999".getBytes();


		byte[] bufferangle=string.getBytes();//should be change that given value from other device
		if(angle!=bufferangle){
			angle=bufferangle;
			sPort.write(angle, 250);
		}
	...
	public static void show(Context context, UsbSerialPort port) {
		sPort = port;
		final Intent intent = new Intent(context, SpydroidActivity.class);
